...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = drop (max 0 (length xs - n)) xs
...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = init ls ++ [x] ++ rs
  where
    (ls, rs) = splitAt (i + 1) xs
...
-- palindromify = until isPalindrome (tail . init)
...
safeIndex xs i | i >= 0 && i < length xs = Just (xs !! i)
               | otherwise               = Nothing
...
eitherDiv x y | y == 0     = Left (show x ++ "/" ++ show y)
